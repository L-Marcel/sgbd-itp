...
Table filter_table(Table table, int column_option, char user_value[200], order filter_order) {
  ...
  if (filter_order == CT) {
    for (i = 0; i < table.qtd_records; i++) {
      ...
      if (strstr(lower_record, lower_value) != NULL) { add_record(&result, table.records[i]); };
    };
  } else {
    for (i = 0; i < table.qtd_records; i++) {
      ...
      switch (filter_order) {
        case GT:
          if (result_order == GT) { add_record(&result, table.records[i]); }
          break;
        case GTE:
          if (result_order == GT || result_order == EQ) { add_record(&result, table.records[i]); }
          break;
        case EQ:
          if (result_order == EQ) { add_record(&result, table.records[i]); }
          break;
        case LTE:
          if (result_order == LT || result_order == EQ) { add_record(&result, table.records[i]); }
          break;
        case LT:
          if (result_order == LT) { add_record(&result, table.records[i]); }
          break;
        default:
          break;
      };
